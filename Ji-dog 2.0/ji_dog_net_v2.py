...
class ActorCritic_Penalty(nn.Module):
    def __init__(self, state_dim, action_dim, action_std_init=0.6):
        super(ActorCritic_Penalty, self).__init__()
    ...
    def compute_loss(self, old_logprobs, logprobs, state_values, rewards, old_values, dist_entropy, advantages, kl_penalty_coef, kl_divergence):
        ratios = torch.exp(logprobs - old_logprobs)
        surr_loss = ratios * advantages
        policy_loss = -surr_loss.mean()
        value_loss = 0.5 * nn.MSELoss()(state_values, rewards)
        kl_penalty = kl_penalty_coef * kl_divergence.mean()
        entropy_bonus = -0.01 * dist_entropy.mean()
        total_loss = policy_loss + value_loss + kl_penalty + entropy_bonus
    
        return total_loss

class PPO_Penalty:
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip, kl_target = 0.5, kl_penalty_coef=0.5, action_std_init=0.6):
        self.gamma = gamma
        ...
        self.policy = ActorCritic_Penalty(state_dim, action_dim, action_std_init).to(device)
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=lr_actor)

        self.policy_old = ActorCritic_Penalty(state_dim, action_dim, action_std_init).to(device)
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            with torch.no_grad():
                kl_divergence = torch.exp(old_logprobs) * (old_logprobs - logprobs)
                kl_mean = kl_divergence.mean()
            
            if kl_mean > self.kl_target * 1.5:
                self.kl_penalty_coef *= 1.5  
